                ...
                <div class="container my-5 py-5 px-lg-5">
                    <div class="row g-5 py-5">
                        <div class="col-12 text-center">
                            <h1 class="text-white animated slideInDown">Kwitansi</h1>
                            <hr class="bg-white mx-auto mt-0" style="width: 90px;">
                            <nav aria-label="breadcrumb">
                                <ol class="breadcrumb justify-content-center">
                                    <li class="breadcrumb-item"><a class="text-white" href="#">Home</a></li>
                                    <li class="breadcrumb-item"><a class="text-white" href="#">Pages</a></li>
                                    <li class="breadcrumb-item text-white active" aria-current="page">Kwitansi</li>
                                </ol>
                            </nav>
                        </div>
                    </div>
...
<!--KWITANSI -->
<div class="card">
        <h5 class="card-header" align="center">Ubah Data Kwitansi</h5>
            <div class="card-body">
        <form method="post" action="proses_ubah_kwitansi.php">
            <?php
                include 'koneksi.php';
                $id_kwitansi=$_GET['id_kwitansi'];
                $qry_get_kwitansi = mysqli_query($conn, "select * from kwitansi where id_kwitansi='".$_GET['id_kwitansi']."'") or die(mysqli_error());
                $dt_kwitansi=mysqli_fetch_array($qry_get_kwitansi);
            ?>
        <input type="hidden" name="id_kwitansi" value= "<?=$dt_kwitansi['id_kwitansi']?>">
          <div class="form-group">
            <label>Diterima dari</label>
            <input type="text" name="dari" value= "<?=$dt_kwitansi['dari']?>" class="form-control">
          </div>
          <div class="form-group">
            <label>Untuk Pembayaran</label>
            <input type="text" name="untuk" value= "<?=$dt_kwitansi['untuk']?>" class="form-control">
          </div>  
          <div class="form-group">
            <label>Sejumlah</label>
            <input type="text" name="jumlah" value= "<?=$dt_kwitansi['jumlah']?>" class="form-control">
          </div>  
          <div class="form-group">
            <label>Terbilang</label>
            <input type="text" name="terbilang" value= "<?=$dt_kwitansi['terbilang']?>" class="form-control">
          </div>  
          <div class="form-group">
            <label>Penerima</label>
            <input type="text" name="penerima" value= "<?=$dt_kwitansi['penerima']?>" class="form-control">
          </div>  
